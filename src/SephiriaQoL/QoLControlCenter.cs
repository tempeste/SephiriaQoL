...
internal sealed partial class QoLControlCenter
{
    ...
    private void DrawLauncher(float scale)
    {
        ...
        GUIStyle launcherStyle = _visible.Value ? OverlayGui.DangerButtonStyle : OverlayGui.SelectedButtonStyle;
        if (GUI.Button(new Rect(100f, 10f, 78f, 32f), _visible.Value ? "Close" : "QoL", launcherStyle))
            _visible.Value = !_visible.Value;
    ...
    {
        OverlayGui.DrawHeader(new Rect(4f, 4f, 512f, 36f));
        GUI.Label(new Rect(16f, 9f, 210f, 26f), "QoL Settings", OverlayGui.TitleStyle);
    ...
    {
        GUIStyle style = _page == page ? OverlayGui.SelectedButtonStyle : OverlayGui.ButtonStyle;
    ...
    private void DrawRunToolsPage()
    {
        ...
        OverlayGui.DrawPanel(new Rect(16f, y + 62f, 488f, 72f));
    ...
    private static float DrawToggle(float y, string title, string description, ConfigEntry<bool> entry)
    {
        ...
        OverlayGui.DrawPip(new Rect(row.x + 12f, row.y + 17f, 10f, 10f),
            entry.Value ? OverlayGui.Success : OverlayGui.MutedText);
        GUI.Label(new Rect(44f, y + 4f, 316f, 20f), title, OverlayGui.LabelStyle);
        GUI.Label(new Rect(44f, y + 23f, 346f, 18f), description, OverlayGui.MutedStyle);

        GUIStyle style = entry.Value ? OverlayGui.SelectedButtonStyle : OverlayGui.ButtonStyle;
        if (GUI.Button(new Rect(424f, y + 10f, 64f, 26f), entry.Value ? "On" : "Off", style))
